...
import { get } from '../lib/utils/auth';
...
async function getNumberOfStreets(locationId: any) {
  ...
  try {
    const res = await fetch(
      `https://hmsapi.herokuapp.com/streets/${locationId}`
    );
                ...
                <Link href={`/locations/${location.id}`}>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-row justify-center items-center text-gray-700">
                      <p className="text-sm text-teal-400 mx-2">
                        Streets: {getNumberOfStreets(location.id)}
                      </p>
                      <span className="text-sm text-teal-400 mx-2">
                        Current workers: Yuri
                      </span>
                    </div>
                  </div>
                </Link>
